...
<div class="body">
    <div class="main-container"> <!-- 리스트와 콘텐츠를 감싸는 컨테이너 -->
        ...
        <c:if test="${lgn.unq_num.toString().startsWith('1') }">
            ...
            <a href="/hb/courseEval?lctr_num=${lctr.lctr_num }"><div class="<%= request.getRequestURI().equals("/hb/courseEval") ? "active" : "" %>">평가</div></a>
            <a href="/hb/showResult?lctr_num=${lctr.lctr_num }&unq_num=${unq_num}"><div class="<%= request.getRequestURI().equals("/hb/showResult") ? "active" : "" %>">성적확인</div></a>
        </c:if>
        
        <c:if test="${lgn.unq_num.toString().startsWith('2') }">
            <a href="/hs/lecProfAttend?lctr_num=${lctr.lctr_num }"><div class="<%= request.getRequestURI().startsWith("/hs/lecProfAttend") ? "active" : "" %>">출석입력(교수)</div></a>
            ...
            <a href="/hb/lecTestResult?lctr_num=${lctr.lctr_num }"><div class="<%= request.getRequestURI().equals("/hb/lecTestResult") ? "active" : "" %>">성적입력(교수)</div></a>
            <a href="/hb/showFinalResult?lctr_num=${lctr.lctr_num }"><div class="<%= request.getRequestURI().equals("/hb/showFinalResult") ? "active" : "" %>">학생수료결과(교수)</div></a>
        </c:if>
        </div>
    </div>
</div>
