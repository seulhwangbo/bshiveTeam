...
<body>
	...
	<c:forEach items="${GongView}" var="board">
	 <c:if test="${writer == board.unq_num }"><button
			onclick="location.href='/mh/gongModify?pst_num=${board.pst_num}'">수정</button></c:if>
		<a href="gongList">목록보기</a>
	 <c:if test="${writer == board.unq_num }"><a href="gongDelete?pst_num=${board.pst_num}" class="delete">삭제</a></c:if>
	</c:forEach>
</body>
</html>
